#include "deepstream_yolov4_app.hpp"
#include <vector>
#include <algorithm>
using namespace std;
...
namespace YOLOv4 {
  ...
  void
  Odin::changeBBoxColor(gpointer obj_meta_data, int has_bg_color, float red, float green,
  ...
  void
  Odin::addDisplayMeta(gpointer batch_meta_data, gpointer frame_meta_data, int status) {
    ...
    if(status==1)
      offset = snprintf(txt_params->display_text, MAX_DISPLAY_LEN, "PERSON UPRIGHT");
    else if(status==0)
      offset = snprintf(txt_params->display_text, MAX_DISPLAY_LEN, "PERSON DOWN");
    else
      offset = snprintf(txt_params->display_text, MAX_DISPLAY_LEN, "NONE");
  ...
  GstPadProbeReturn
  Odin::tiler_src_pad_buffer_probe(GstPad *pad, GstPadProbeInfo *info,
                            gpointer u_data) {

     vector<Point> vert(4);
    vert[0] = Point( 474, 858 );
    vert[1] = Point( 1203, 533 );
    vert[2] = Point( 1294, 554 );
    vert[3] = Point( 727, 1033 );
    ...
    for (l_frame = batch_meta->frame_meta_list; l_frame != NULL;
     ...
     int status = -1;
      ...
      if(obj_meta->class_id==0 ){
         ...
         obj_cen.x=int(obj_meta->rect_params.left);
              obj_cen.y=int(obj_meta->rect_params.top+((obj_meta->rect_params.height)));
    ...
    //  addDisplayMeta(batch_meta, frame_meta, status);
     display_meta = nvds_acquire_display_meta_from_pool(batch_meta);
        display_meta->num_labels = 1;
  ...
  Odin::configure_element_properties(int num_sources, GstElement *streammux, GstElement *pgie_yolo_detector,
    ...
    if (boost::filesystem::exists(boost::filesystem::path(
        YOLOv4::Odin::PGIE_YOLO_ENGINE_PATH))) {

      g_object_set(G_OBJECT(pgie_yolo_detector),
                  "model-engine-file", YOLOv4::Odin::PGIE_YOLO_ENGINE_PATH.c_str(), NULL);
    }
    else {
    ...
    if (!YOLOv4::Odin::set_tracker_properties(nvtracker)) {
  ...
  void Odin::setPaths(guint batch_size) {
    ...
    strdup("models/YOLOv4/config_infer_primary_yolov4.txt");
    ...
    str(boost::format("models/YOLOv4/yolov4.weights_b%d_gpu0_%s.engine") % batch_size % COMPUTE_MODE);
  }
}
